	...
	with open(out_path, 'a') as outf:
		...
		while start_idx + prev_hour + next_hour <= num_hour:
			...
			for key in keep_feats:
				feat_list = feats[key]
				if key == target:
					if feat_list[start_idx + prev_hour] == "-1":
						feat_list[start_idx + prev_hour] = feat_list[start_idx + prev_hour -1]
					labels = feat_list[start_idx + prev_hour]

				if key not in keep_feats:
					continue

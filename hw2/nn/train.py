...
def gbdt_train(train_data, train_label, val_data, val_label, tree_depth=3, num_round=30, tuned=True):
	...
	if tuned is True:
		bst = XGBClassifier(learning_rate=0.1, n_estimators=500, max_depth=6,
															min_child_weight=1, gamma=0, subsample=0.8, colsample_bytree=0.65,
															objective= 'binary:logistic', nthread=16, scale_pos_weight=1, seed=27)

		bst.fit(train_data, train_label, eval_metric="error", eval_set=[(train_data, train_label), (val_data, val_label)], verbose=True, early_stopping_rounds=50)
		ypred = bst.predict(val_data)
	else:
		param = {'max_depth': tree_depth, 'eta': 1, 'silent': 0, 'objective': 'binary:logistic'}
		param['nthread'] = 4
		...
		bst = xgb.train(param, train_data_lbl, num_round, evallist)
